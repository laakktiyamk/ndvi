 ...
 */
const getPixelData = async (response: Buffer, scaleColors: string[]): Promise<ColorItem[]> => {

  /**
   * Reads the image and extracts the RGBA values for each pixel.
   */
  async function getRGBArray(imgBuffer: Buffer): Promise<number[][]> {
    try {
   ...
   */
  function getUniquePixels(image: number[][]): number[][] {
    return image.reduce((unique: number[][], color: number[]) => {
   ...
   */
  function getCounts(colors: number[][], pixel: number[]): number[][] {
    return colors.filter((item) => item.toString() === pixel.toString());
  }

  const counts: number[] = uniquePixels.map((pixel) => {
    return getCounts(rgbArray, pixel).length;
  });

  let greenColorStr: string = scaleColors[scaleColors.length - 1];
  const notGreenColors: string[] = [...scaleColors];
  notGreenColors.pop();

  /**
   * Converts an RGBA array to an RGB string.
   */
  function getRgbString(color: number[]): string {
  ...
  const greenColor: number[] = [...values, 255];
  ...
  const percentages: number[] = res.map((item) => {
